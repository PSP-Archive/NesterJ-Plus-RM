...
void load_config(void)
{
	...
	if(fd>=0){
		...
		//<<<davex		
		// 娙堈僄儔乕僠僃僢僋(g_NESConfig)
		if (unSizeOfNESConfig != sizeof(g_NESConfig)) goto SetDefault;
		return;
	}
SetDefault:
	MessageBox("Load config FAILED. Set default...", RGB(255,0,0), MB_OK);
...
void menu_frame(const char *msg0, const char *msg1)
{
	...
	//mh_print(314, 1, " 仭 NesterJ for PSP Ver1.12 仭", setting.color[1]);
	//mh_print(310, 1, " 仭 NesterJ for PSP Plus 0.5 仭", setting.color[1]);
	mh_print(303, 1, " 仭 NesterJ for PSP Plus 0.61 RM 仭", setting.color[1]);
...
void menu_frame_cn(const char *msg0, const char *msg1)
{
	...
	if(g_bEnableBitmap)
		...
		pgFillvram(setting.color[0]);
	//mh_print(314, 1, " 仭 NesterJ for PSP Ver1.12 仭", setting.color[1]);
	mh_print(306, 1, " 仭 NesterJ for PSP Plus 0.61 仭", setting.color[1]);
	GetBatteryInfoString(msg, &color);
	mh_print(464-(_strlen(msg)*5), 254, msg, color);
	// 儊僢僙乕僕側偳
	if(msg0!=0) mh_print(17, 12, msg0, setting.color[2]);
	...
	pgDrawFrame(18,24,462,249,setting.color[1]);
	// 憖嶌愢柧
	if(msg1!=0) mh_printGBK12(17, 254, msg1, setting.color[2]);
}
	...
	for(;;){
		...
		if(new_pad & PSP_CTRL_CIRCLE){
			if(sel == SAVE_SLOT){
				...
				_strcpy(szMsg, "Are you sure to DETELE slot -");
				_itoa(StateSlot, szTmp);
				_strcat(szTmp, " ?");
				_strcat(szMsg, szTmp);
				if (MessageBox(szMsg, RGB(255, 64, 64), MB_OKCANCEL) == IDOK) {
				...
				if (MessageBox("Are you sure to save the SCREEN SHOT ?", RGB(64, 64, 255), MB_OKCANCEL) == IDOK) {
					if (PSPEMU_SaveScreenShot()) {
						_strcpy(msg, "Save the Screen Shot Successfully");
					}
					else {
						_strcpy(msg, "Save the Screen Shot Failed");
					}
				}
			...
			}else if(CHEAT_CONFIG==sel)
			{			
				...
				if (nRet >= 0){
					...
					MessageBox("Unsupported board. Pls mail to: tensai_wang@msn.com",RGB(255,0,0), 1);
				}
			}else if(sel == RESET){
...
boolean nesterj_menu_cn(void)
{
	...
	for(;;){
		readpad();
		if (nNoKeyCount) {nNoKeyCount--; new_pad = 0;}
		if(new_pad & PSP_CTRL_CIRCLE)//按键（O）判断过程
		{
			if(sel == SAVE_SLOT)
			{
				StateSlot++;
			...
			{
				_strcpy(szMsg, "您确实要保存到档案 - ");
				_itoa(StateSlot, szTmp);
				_strcat(szTmp, " 吗?");
				_strcat(szMsg, szTmp);
				if (MessageBoxGbk12(szMsg, RGB(255, 64, 64), MB_OKCANCEL) == IDOK) {
					if(PSPEMU_SaveState(StateSlot)) {
						_strcpy(msg, "已成功保存即时存档");
						update_stateslot(StateSlot);
					}
					else {
						_strcpy(msg, "保存即时存档失败");
					}
				}
			}
			else if(sel == STATE_LOAD && g_Slot[StateSlot].bSlotExist)
			{
				_strcpy(szMsg, "您确实要载入档案 - ");
				_itoa(StateSlot, szTmp);
				_strcat(szTmp, " 吗?");
				_strcat(szMsg, szTmp);
				if (MessageBoxGbk12(szMsg, RGB(64, 64, 255), MB_OKCANCEL) == IDOK) {
					if(PSPEMU_LoadState(StateSlot))
						break;
					else
						_strcpy(msg, "即时读档失败");
			...
			else if(sel == STATE_DELETE && g_Slot[StateSlot].bSlotExist)
			{
				...
				if (MessageBoxGbk12("您确实要保存截图吗 ?", RGB(64, 64, 255), MB_OKCANCEL) == IDOK) {
					if (PSPEMU_SaveScreenShot()) {
						_strcpy(msg, "已成功保存截图");
					}
					else {
				...
				submenu_diskchange();
				msg[0]=0;
			}else if(sel == EMU_CONFIG){
				//emu_preferencesconfig();
				emu_config_cn();
				msg[0]=0;
			}else if(sel == CHEAT_GAME_GENIE){
				int nRet;
			...
			{
				if (MessageBoxGbk12("您确实要重置游戏吗 ？", RGB(255, 0, 0), MB_OKCANCEL) == IDOK) {
					// リセット
					NES_reset(0);
					break;
				}
			...
			{
				if (MessageBoxGbk12("您确实要退出模拟器，返回 PSP 桌面吗？", RGB(255, 0, 0), MB_OKCANCEL) == IDOK) {
					bRet = FALSE;
					break;
				}
			}
		...
		else if(new_pad & PSP_CTRL_AD_UP)//上
		{
			if(sel!=0)
				sel--;
			else
		...
		menu_frame_cn(msg, "○：确定  ×：取消");

		if (sel == SCREEN_SHOT) 
		{
			pgDrawFrame(204,24,205,25+224,setting.color[1]);
			DrawCurrentScreen(206,25);
			mh_printGBK12(400,236,"当前屏幕",setting.color[2]);
		}
		else 
		{
			//pgPrint(30, 4,setting.color[2], "State Save List");
			mh_printGBK12(264,28,"即时存档列表",setting.color[2]);
			for (cbI = 0; cbI < STATE_SLOT_MAX; cbI++) {
				int nColorIndex = (StateSlot == cbI) ? 3: 2;
				pgPrint(33, cbI + 5,setting.color[nColorIndex], g_Slot[cbI].szData);
			}
			...
			if (g_Slot[StateSlot].AvailableThumbnail == AVAILABLE) {
				pgDrawFrame(330-1,130-1,330+128,130+112,setting.color[1]);
				pgBitBltAlpha75(330,130,128,112,(uint16*)g_Slot[StateSlot].thumbnail);
			}
		}
		...
		_itoa(StateSlot, szTmp);
		_strcpy(szSlotNum, "   即时存档档案  ： ");
		_strcat(szSlotNum, szTmp);
		pgPrintGBK12(x,y++,setting.color[3], szSlotNum);
		pgPrintGBK12(x,y++,setting.color[3],"   即时存档");
		pgPrintGBK12(x,y++,setting.color[3],"   即时读档");
		pgPrintGBK12(x,y++,setting.color[3],"   删除存档");

		mh_printGBK12(x*6,y*12+6,"   游戏截图",setting.color[3]);y+=2;

		pgPrintGBK12(x,y++,disksidenum ? setting.color[3]: setting.color[2],"   换磁碟");
		
		mh_printGBK12(x*6,y*12+6,"   模拟器设置",setting.color[3]);y+=2;

		pgPrintGBK12(x,y++,setting.color[3],"   载入 Game Genie(作弊) 文件");
		pgPrintGBK12(x,y++,setting.color[3],"   载入 Cheat(作弊) 文件");
		pgPrintGBK12(x,y++,NES_GetCheatCodeNum() ? setting.color[3]:setting.color[2],"   配置作弊项");

		mh_printGBK12(x*6,y*12+6,"   返回到 ROM 列表",setting.color[3]);y++;
		mh_printGBK12(x*6,y*12+6,"   重置游戏",setting.color[3]);y++;
		mh_printGBK12(x*6,y*12+6,"   继续游戏",setting.color[3]);y++;
		y++;
		pgPrintGBK12(x,y++,setting.color[3],"   退出模拟器");//Exit to PSP Menu

		if(sel<4) y=38+12*sel;
		else if(sel==4)y=92;//截图
		else if(sel==5)y=110;
		else if(sel==6)y=128;
		else if(sel==7)y=146;//作弊
		else if(sel==8)y=158;
		else if(sel==9)y=170;
		else if(sel==10)y=188;
		else if(sel==11)y=200;
		else if(sel==12)y=212;
		else if(sel==13)y=230;

		if (nCursor/5) pgPutChar(x*8,y,setting.color[3],0,127,1,0,1);//显示选择箭头 nCursor/5显示间隔
